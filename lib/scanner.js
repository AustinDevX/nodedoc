...
function getClassDocumentation(docs, className) {
  ...
  debug(`Matching classname to modulename ${moduleName}`);
...
function getMethodDocumentation(documentation, methodName){
  debug(`Retrieving documentation for event : ${methodName}`);
...
function getEventDocumentation(documentation, eventName){
  debug(`Retrieving documentation for event : ${eventName}`);
  let events  = ext.extractEvents(documentation).val;
  let match   = misc.findPropertyMatch(events, {name: eventName}).val;
  return maybe.fromNull(match);
}
